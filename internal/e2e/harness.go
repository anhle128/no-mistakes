...
func NewHarness(t *testing.T, opts SetupOpts) *Harness {
	...
	// Keep the harness root short: Darwin rejects Unix socket paths once the
	// full nmhome/socket path exceeds its sockaddr_un limit.
	testRoot, err := os.MkdirTemp("", "nm-e2e-")
	if err != nil {
	...
	h := &Harness{
		...
		NMHome:            filepath.Join(testRoot, "nmhome"),
		HomeDir:           filepath.Join(testRoot, "home"),
		UpstreamDir:       filepath.Join(testRoot, "upstream.git"),
		WorkDir:           filepath.Join(testRoot, "work"),
		AgentLog:          filepath.Join(testRoot, "fakeagent.log"),
