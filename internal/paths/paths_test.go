...
func TestReportPaths(t *testing.T) {
	...
	if got := p.RunReportDir("run1"); got != filepath.Join(root, "reports", "run1") {
		t.Errorf("RunReportDir() = %q", got)
	}
	if got := p.RunReviewResolutionReportPath("run1"); got != filepath.Join(root, "reports", "run1", "review-resolution.md") {
		t.Errorf("RunReviewResolutionReportPath() = %q", got)
	}
}
