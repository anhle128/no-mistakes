...
- **AXI/TUI display**: Show compact status, counts, and repo-local report path only; keep the full narrative in the Markdown report.
- **PR content**: Include compact counts/status and the repo-relative committed report path; never publish absolute local paths, report excerpts, or private filesystem details.
...
This spec is derived from [plans/grill-me/review-resolution-report.md](../../plans/grill-me/review-resolution-report.md). The full `## Decisions` section in that file is binding origin context for this feature. Any prior note that treated the grill-me file as superseded was incorrect; later phases must preserve the repo-local committed report direction unless a new explicit decision record changes it.
...
- The durable Markdown artifact belongs in the target repository at `no-mistakes/<branch-slug>/review-resolution.md`.
- The branch slug keeps safe nested folders split on `/`, drops unsafe/path-traversal segments, and falls back to the run ID when no safe branch segment remains.
...
- The push step must force-add exactly the current run's repo-local report artifact when it exists, even if `no-mistakes/` is ignored, without force-adding unrelated `no-mistakes/` files.
- Source anchors: `plans/grill-me/review-resolution-report.md` lines 45-89 and 314-344.

### Historical Requirements Not Carried Forward

- None of the repo-local grill-me report-path decisions are rejected by this spec.
- Full one-issue/one-commit scheduling, amend-on-retry behavior, and per-issue auto-fix budgeting from lines 144-205 remain product requirements that may require later implementation hardening beyond this report-path correction.
- The only out-of-scope expansion remains all-step evidence reporting; this version stays Review-only.
...
- Default behavior: create the repo-local committed report when Review findings exist, omit it for clean review from lines 331-341.
- Minimum coverage expectations for report generation, push staging, and PR/summary surfacing from lines 388-404.
...
**Independent Test**: Run a gate scenario where Review returns two findings, fix one, and approve the other. The run produces `no-mistakes/<branch-slug>/review-resolution.md` in the run checkout with both findings in separate outcome sections.
...
3. **Given** a Review step emits findings, **When** the run is later inspected through TUI or AXI, **Then** those surfaces expose that a review-resolution report exists and provide the repo-local report path or compact status.
...
As a reviewer reading the generated PR body, I want a concise indication that Review findings were resolved, accepted, or left open, so the PR summary points me to the committed resolution evidence without exposing absolute local filesystem paths.
...
**Independent Test**: Build PR content for runs with and without a review-resolution report. The PR body includes review-resolution status only when the report exists, references the repo-relative report path, and never emits an absolute local path.

**Acceptance Scenarios**:

1. **Given** a review-resolution report exists, **When** PR content is generated, **Then** the `## Pipeline` section includes compact review-resolution counts/status from metadata and the repo-relative report path.
...
3. **Given** the metadata stores an absolute checkout path, **When** PR content is generated, **Then** the PR body extracts only the `no-mistakes/<branch-slug>/review-resolution.md` repo-relative path and does not publish the absolute checkout path.
...
- The repo-local `no-mistakes/<branch-slug>/review-resolution.md` path cannot be written or report metadata cannot be persisted: the Review step/run must fail with an actionable error; it must not silently claim report generation succeeded.
...
- **FR-001**: System MUST create a durable Markdown review-resolution report at `no-mistakes/<branch-slug>/review-resolution.md` inside the run checkout immediately after the Review step first records one or more findings.
- **FR-002**: System MUST derive `<branch-slug>` with the grill-me safety model: strip `refs/heads/` when present, split branch names on `/`, keep only `A-Z`, `a-z`, `0-9`, `-`, `_`, and `.`, replace other characters with `-`, collapse repeated `-`, trim leading/trailing `-`, drop empty, `.`, and `..` segments, and fall back to the run ID if no safe segment remains.
- **FR-002A**: System MUST stage or force-add exactly the current run's repo-local review-resolution report before push/PR evidence is sent, even when `no-mistakes/` is ignored, and MUST NOT stage unrelated files under `no-mistakes/` because of this feature.
...
- **FR-004**: System MUST include a `Run Context` section with run ID, repo identifier or path, branch, base commit, current/final head commit at generation time, Review step status, report lifecycle state, first generated timestamp, last refreshed timestamp, finalized timestamp when applicable, and repo-local report path.
...
- **FR-017**: System MUST persist compact report metadata in a dedicated SQLite `review_resolution_reports` table keyed by `run_id`, sufficient for AXI, TUI, and PR summary code to know whether a report exists, where the repo-local artifact is located, when it was generated, its surface-facing status, and how many issues are resolved, accepted, informational, and still open.
...
- **FR-022**: System MUST expose compact report status, issue counts, and repo-local report path through AXI and TUI run detail surfaces when a report exists, and MUST NOT inline the full report narrative in those surfaces. Surface wording MUST be derived from the persisted report status and counts; any nonzero `still_open_count` or evidence-unavailable status MUST use non-success language.
- **FR-023**: System MUST update generated PR content to reference compact review-resolution counts/status and the repo-relative committed report path from metadata when a report exists, without publishing absolute local filesystem paths, report excerpts, or private filesystem details.
...
- **Review Resolution Report**: Repo-local durable Markdown artifact for one run, stored at `no-mistakes/<branch-slug>/review-resolution.md` inside the run checkout and intended to be committed as PR evidence.
...
- **Report Reference**: Surface-specific pointer to the report or its status. AXI/TUI show compact status, counts, and repo-local path; PR summaries show compact counts/status and the repo-relative report path while avoiding absolute local filesystem links or report excerpts.

## Constitution Alignment *(mandatory)*

- **Gate Semantics**: The feature preserves the fixed gate order and does not change what it means for a branch to pass. It adds Review evidence after findings occur and does not alter `origin` behavior.
- **Isolation/User Control**: Intentional report writes go under repo-local `no-mistakes/<branch-slug>/` because the report is committed PR evidence; SQLite metadata remains local runtime support. The report reflects user approval, skip, fix, and abort decisions without inventing acceptance.
...
- **SC-007**: PR body generation tests show compact review-resolution counts/status and repo-relative report path when metadata exists, omit the section when no report exists, and never include absolute local paths or report excerpts. Integration tests MUST compare the Markdown section counts, SQLite metadata counts/status, AXI/TUI run-detail output, and PR summary output for the same run to prove they come from the same persisted classified snapshot.
...
- The target repository's `no-mistakes/<branch-slug>/review-resolution.md` path is the durable artifact boundary for this report; `$NM_HOME` remains runtime state for DB/logs only.
- PR reviewers benefit from compact report status in the PR body plus a repo-relative pointer to the committed full report.
...
- `internal/reviewreport/refresh.go`: repo-local report path derivation, safe branch path handling, atomic report refresh, and metadata upsert.
