...
Create a durable Review-only resolution report whenever the Review step records findings. The implementation will rebuild report state from persisted Review step rounds, classify each normalized finding once, write a local Markdown artifact under `$NM_HOME/reports/<runID>/review-resolution.md`, persist compact metadata in SQLite, and expose only compact status/counts/path through local AXI/TUI surfaces while keeping PR content free of local filesystem details.

The core design is a new review-report domain package backed by additive DB and path helpers. The pipeline executor refreshes the report at Review lifecycle boundaries and before PR summary generation. The Review fix agent contract remains backward-compatible with the existing `summary` field while accepting validated optional `resolutions[]` details.
...
**Storage**: `$NM_HOME/state.sqlite`, `$NM_HOME/reports/<runID>/review-resolution.md`, existing step results/rounds, local logs/evidence directories
...
**Constraints**: Do not modify `origin`; do not create, stage, force-add, or commit repo-local review reports; report writes and metadata writes must be atomic from consumer perspective; report content must not store raw transcripts, raw logs, raw hunks, code blocks, or secret-like values
...
- **Isolation and User Control**: PASS. Writes are bounded to `$NM_HOME/reports/<runID>/` and SQLite state. Acceptance without fix requires persisted user or documented policy provenance; aborted/superseded/canceled runs keep unresolved findings still open.
...
```text
internal/reviewreport/        # New report classification, sanitization, rendering, atomic refresh
internal/db/                  # Additive schema, report metadata accessors, fix/decision evidence persistence
internal/paths/               # ReportsDir and ReviewResolutionReportPath helpers
internal/pipeline/            # Executor lifecycle hooks for report refresh/failure propagation
internal/pipeline/steps/      # Review fix schema extension and PR summary integration
...
**Structure Decision**: Add a focused `internal/reviewreport` package rather than spreading classification/rendering rules across executor, PR summary, AXI, and TUI. DB remains the compact state authority; Markdown is the human-readable local artifact.
...
- Keep PR content local-path-free and narrative-free; AXI/TUI may show the local report path.
...
1. **Persistence and paths**
   - Extend `internal/paths.Paths` with `ReportsDir()` and `ReviewResolutionReportPath(runID string)`.
...
5. **Surfaces**
   - Extend IPC `RunInfo` or a nested report metadata object with status, counts, and local path.
   - AXI and TUI render compact status/counts/path only when a report exists.
   - PR summaries render compact status/counts only. They must never include local paths, report excerpts, or generated GitHub blob links for `$NM_HOME` artifacts.
...
1. DB/path foundation and migrations.
...
- `internal/pipeline/steps`: PR summary includes compact metadata only and omits local paths/excerpts.
